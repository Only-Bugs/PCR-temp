 ...
 * Includes SharePosterModal for sharing achievements.
 *
 * @param {Object} props
...
const CarbonCard = ({ data }) => {
  ...
  const stage = Math.floor(points / 500) + 1;
  const pointsToNextMilestone = 500 - (points % 500);

  // Estimate CO₂ saved (placeholder formula)
  const co2SavedKg = (points / 18).toFixed(1);
